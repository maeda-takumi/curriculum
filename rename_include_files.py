#!/usr/bin/env python3
"""include配下の01_files〜51_files内のファイル名を一括リネームするスクリプト。"""
...
def planned_name(file_path: Path) -> str | None:
    ...
    if name.endswith(".js.ダウンロード"):
        return name[: -len(".ダウンロード")]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="include/01_files〜51_files配下のファイルを仕様に沿ってリネームします。"
    )
    parser.add_argument(
        "--base-dir",
        type=Path,
